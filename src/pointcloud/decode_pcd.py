...
from src.pointcloud.point_cloud import PointCloud

TO_M = 0.001


def tuple_to_ordered_lists(data_tuple, fields=None):
    "return a point coordinates in the x, y, z order"
    ...
    for i in range(0, len(data_tuple), 3):
        sublist = [
            data_tuple[i + fields.index("x")],
            data_tuple[i + fields.index("y")],
            data_tuple[i + fields.index("z")],
        ]
...
def decode_pcd_bytes(pcd_bytes, min_range_mm=None) -> PointCloud:
    ...
    if metadata["DATA"] != "binary":
        raise ValueError(
            f"DATA format is not supported. DATA should be binary but got: {metadata['DATA']}"
        )
